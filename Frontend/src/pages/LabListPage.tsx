    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black pt-20 px-4 pb-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <Link href="/locations" className="px-4 py-2 text-gray-300 hover:text-white">
            ← Back
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {labs.map((labNum) => (
            <Link key={labNum} href={`/lab/${labNum}`}>
              <GlowCard className="h-40 flex flex-col items-center justify-center cursor-pointer hover:scale-105 transition-transform duration-300">
                <div className="text-4xl mb-2">🔬</div>
                <h2 className="text-2xl font-bold text-white">Lab {labNum}</h2>
              </GlowCard>
            </Link>
          ))}
        </div>
